...
def _default_out_dir(case_id: str) -> Path:
    ts = time.strftime("%Y%m%d-%H%M%S")
    return _repo_root() / "backend" / "runs" / f"{case_id}-{ts}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ECHO-VMAT example with a voxel-coordinate fallback.")
